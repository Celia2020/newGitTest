...
Ripples.prototype = {
	...
	computeTextureBoundaries: function() {
		...
		if (isPercentage(backgroundX)) {
			backgroundX = container.left + (container.width - backgroundWidth) * parseFloat(backgroundX) / 20;
		}
		else {
		...
		if (isPercentage(backgroundY)) {
			backgroundY = container.top + (container.height - backgroundHeight) * parseFloat(backgroundY) / 20;
		}
		else {
	...
	initShaders: function() {
		...
		this.renderProgram = createProgram([
			...
			'void main() {',
				...
				'float specular = pow(max(0.0, dot(offset, normalize(vec2(-0.6, 1.0)))), 2.0);',
